...
int main() {
    // 配置
    TtsConfig config = TtsConfig::MatchaZH();
...
"""
import evo_tts
from evo_tts import Engine, Config, Result, BackendType, TtsCallback

# =============================================================================
# 快捷函数
# =============================================================================

def synthesize(text: str, model_dir: str = "~/.cache/matcha-tts") -> Result:
    """
    合成文本 (一行代码)
...
def synthesize_to_file(text: str, file_path: str, model_dir: str = "~/.cache/matcha-tts") -> bool:
    """
    合成文本并保存到文件
...
class Config:
    ...
    def __init__(self, backend: BackendType = BackendType.MATCHA_ZH,
                 model_dir: str = "~/.cache/matcha-tts"):
        """
        Args:
            backend: 后端类型
            model_dir: 模型目录路径
    ...
    @staticmethod
    def matcha_zh(model_dir: str = "~/.cache/matcha-tts") -> "Config":
        """创建 Matcha 中文配置 (22050Hz)"""

    @staticmethod
    def matcha_en(model_dir: str = "~/.cache/matcha-tts") -> "Config":
        """创建 Matcha 英文配置 (22050Hz)"""

    @staticmethod
    def matcha_zh_en(model_dir: str = "~/.cache/matcha-tts") -> "Config":
        """创建 Matcha 中英混合配置 (16000Hz)"""
...
# 快捷方式
evo_tts.synthesize_to_file("你好世界", "output.wav")

# Engine 类
config = evo_tts.Config.matcha_zh()
config.speech_rate = 1.0

with evo_tts.Engine(config) as engine:
    # 文本合成
...
# 链式配置
config = evo_tts.Config.matcha_en().with_speed(1.2).with_volume(80)

# 多语言
config_en = evo_tts.Config.matcha_en()      # 英文
config_zh_en = evo_tts.Config.matcha_zh_en()  # 中英混合
```
...
```python
from evo_tts import TtsCallback
...
# 配置音频播放 (22050Hz, mono)
evo_audio.init(sample_rate=22050, channels=1)

with AudioPlayer() as player:
    player.start()

    with evo_tts.Engine() as engine:
...
```python
from evo_tts import PrintCallback, SaveCallback, CollectCallback
