...
public:
    ...
    // Ensure cppjieba is cloned to ~/.cache/cppjieba
    bool ensureCppJieba();

    // Ensure cpp-pinyin is cloned to ~/.cache/cpp-pinyin
    bool ensureCppPinyin();

    // Get cppjieba dictionary path (~/.cache/cppjieba/dict)
    std::string getCppJiebaPath() const;

    // Get cpp-pinyin dictionary path (~/.cache/cpp-pinyin/res/dict)
