...
void synthesisThread(Evo::TtsEngine& engine,
...
void playbackThread(AudioQueue& queue, bool enable_play, int output_rate, int channels) {
    std::unique_ptr<EvoAudio::AudioPlayer> player;

    if (enable_play) {
        player = std::make_unique<EvoAudio::AudioPlayer>(-1);
...
Evo::BackendType parseLanguage(const std::string& lang) {
    if (lang == "en") {
        return Evo::BackendType::MATCHA_EN;
    } else if (lang == "zh") {
        return Evo::BackendType::MATCHA_ZH;
    }
    return Evo::BackendType::MATCHA_ZH_EN;
}
...
int main(int argc, char* argv[]) {
    ...
    int sample_rate = (backend == Evo::BackendType::MATCHA_ZH_EN) ? 16000 : 22050;
    ...
    Evo::TtsConfig config;
    config.backend = backend;
    config.sample_rate = sample_rate;

    Evo::TtsEngine engine(config);
